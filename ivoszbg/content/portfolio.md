...
List of hardware that I've brought up from the zero.
...
This was actually relatively hard to bring up. There are no absolutely no sources, nor any kind of documentations for this platform. Both Powkiddy and Actions Semiconductor ignored my emails, so I had to reverse-engineer a lot of things. Thankfully I managed to also find ancient sources for a somewhat similar platform, from where I borrowed information about registers.
...
- the keys aren't wired through GPIO, but the pmic. Aaaaaand that's bad since we only have regulators functionality for the pmic upstream.
...
This has been my biggest project so far. It's reached a somewhat mature point, where devices are usable for media consumption (without audio though :P), with important hardware like Decon, GPU, touchscreen and WIFI/BT working. Nevertheless it still has some issues that need resolving, like:
- the SMP isn't fully working and wayland DEs like phosh are being a bit unstable.
- USB is a bit quirky, I've gotten the PHY to work but dwc doesn't work.
...
Exynos8895 is definitely not an easy platform to work on. As one of the high-end modern exynos SoCs (even for 2017), it has complex peripherals and a large amount of buses like I2Cs, UARTs, HSI2Cs. Here's a small list of things I'm planning to do in the future:
- a driver for the new SPEEDY bus, which is used for PMIC
- 8895 features usbdrd3.0 that needs somewhat complex PHY tuning.
- the UFS phy, which also has been causing me some trouble.
...
---
# uniLoader
[uniLoader](https://github.com/ivoszbg/uniLoader) is a secondary bootloader that is capable of loading the upstream Linux kernel for Android and iOS-based devices.
